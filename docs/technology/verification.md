...
| G — XRP-native funding | XRPL payment, FDC proof, Smart Account deposit | LOCAL FUNDING CHECKPOINTS + PUBLIC `PersonalAccount`/nonce/`0xFE`/`XRPPayment` REQUEST CODEC + INTEGER FASSETS QUOTE/CORE-VAULT ADDRESS + VALIDATED XRPL READ + STRICT FDC PREPARE/DA ENVELOPE/FINALITY/FEE-INTENT/DIRECT-MINT-CALL PASS / LIVE PAYMENT, FDC SUBMISSION/MERKLE LEAF VERIFICATION, AND SMART ACCOUNT DEPOSIT NOT VERIFIED |
