...
| B — Private policy ingress | sealed policy, three receipts, replay/domain negatives | LIVE SIMULATED PASS — THREE INDEPENDENT ECIES WRITES + THREE MACHINE-SIGNED RECEIPTS + SANITIZED ON-CHAIN FREEZE / HARDWARE + REPLACEMENT OPEN |
...
| H — Product release | full roles, recovery, accessibility, live deployment | INTERACTIVE VERCEL DAPP + LIVE RAILWAY SIMULATED-FCC V1 LIFECYCLE + PUBLIC-SAFE REPOSITORY EVIDENCE + LANDING + DESKTOP/MOBILE/KEYBOARD BROWSER SMOKE / HOSTED-WEB INTEGRATION, HARDWARE, V2, OUTAGE, AND RELEASE MANIFEST OPEN |
