...
| Stateless threshold relay/executor core | `NEW` | `apps/relay/` | `916f140`, `1a8c6f0`, `4101e56`, `987db37`, `a6788a0`, `546c641` | Threshold/domain/replay/split/outage, idempotency, rate limit, public recovery, competing executor, health, aggregate-private observability, and a hosted authenticated V2 Coston2 candidate path. The deployed relay accepts ciphertext only at ingress, reconstructs evaluation state from chain, rejects a client decision, and remains simulated/operator-only rather than a verified release. |
