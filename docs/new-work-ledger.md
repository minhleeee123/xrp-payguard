...
| Stateless threshold relay/executor core | `NEW` | `apps/relay/` | `916f140`, `1a8c6f0`, `4101e56`, `987db37`, `a6788a0`, `546c641` | Threshold/domain/replay/split/outage, idempotency, rate limit, public recovery, competing executor, health, aggregate-private observability, and a hosted authenticated V2 Coston2 candidate path. The local upgrade accepts the exact policy-owner evaluation signature with owner/IP budgets while keeping the relay as a bounded gas-paying executor; the currently deployed build remains simulated/operator-only until redeployed and re-verified. |
...
| Web application and landing | `NEW` implementation from the adapted visual direction | `apps/web/`, `DESIGN.md` | `81f4797`, `8383d18`, `d597036`, `6cc99c2`, `5e15726`, `914a7bb`, `46ac651` | Unit/type/build, desktop/mobile/keyboard/reduced-motion smoke, reviewed evidence decoding, Lighthouse, no browser persistence, Vercel asset reachability, and hosted V2 FCC custody/evaluation controls. The local UI now supports connected-wallet ownership from Policy Studio through Requests; production remains the earlier simulated/operator-only build until the requested batch deployment. |
