...
import { actionRequestHash, evaluationAttestationDigest, evaluationDigest, fdcTriggerSnapshotCommitmentV1, policyBindingDigest, policyCommitment, policyIngressAuthorizationDigest, policyReceiptAttestationDigest, policyReceiptDigest } from "../src/codec.js";
import { evaluatePolicy } from "../src/evaluator.js";
import type { ActionRequestV1, FdcTriggerSnapshotV1, Hex, PolicyBindingV1, PolicyReceiptV1, PolicyV1, SpendStateV1 } from "../src/types.js";
...
const vector = JSON.parse(readFileSync(resolve(import.meta.dirname, "../fixtures/v1.json"), "utf8")) as {
  ...
  result: Record<string, unknown>; fdcSnapshot: Record<string, unknown>; expected: Record<string, string>;
};
const bigintFields = ["chainId", "maxPerAction", "dailyCap", "rollingCap", "rollingWindowSeconds", "startAt", "endAt", "scheduleIntervalSeconds", "scheduleGraceSeconds", "cooldownSeconds", "maxPriceAgeSeconds", "fdcMinReceivedAmount", "fdcMaxReceivedAmount", "maxFdcAgeSeconds"];
...
const fdcSnapshot = { ...vector.fdcSnapshot } as unknown as FdcTriggerSnapshotV1;
for (const field of ["receivedAmount", "blockNumber", "blockTimestamp"]) {
  (fdcSnapshot as unknown as Record<string, unknown>)[field] = BigInt(fdcSnapshot[field as keyof FdcTriggerSnapshotV1] as string);
}
...
describe("cross-language golden vector", () => {
  it("matches policy, receipt, request, and result domains", () => {
    ...
    expect(policyBindingDigest(binding)).toBe(vector.expected.bindingDigest);
    ...
    expect(policyIngressAuthorizationDigest({
      binding, submissionNonce: receipt.submissionNonce, issuedAt: receipt.issuedAt,
      expiry: receipt.expiry, ciphertextHash: vector.expected.ingressCiphertextHash as Hex,
      machineId: receipt.machineId, keyFingerprint: receipt.keyFingerprint,
    })).toBe(vector.expected.ingressAuthorizationDigest);
    expect(actionRequestHash(request)).toBe(vector.expected.requestHash);
    expect(fdcTriggerSnapshotCommitmentV1(fdcSnapshot)).toBe(vector.expected.fdcSnapshotCommitment);
