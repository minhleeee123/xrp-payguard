...
function localEntries() {
  const coston2 = Array.from({ length: 21 }, (_, index) => ({
...
describe("public web deployment evidence audit", () => {
  ...
  it("verifies every deployed JSON body byte-for-byte against reviewed sources", async () => {
    ...
    assert.deepEqual(observation.counts, { total: 24, chain114: 23, simulation: 3 });
    ...
    assert.equal(evidence.corpus.entries.length, 24);
