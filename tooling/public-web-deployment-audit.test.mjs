...
function localEntries() {
  const coston2 = Array.from({ length: 11 }, (_, index) => ({
...
describe("public web deployment evidence audit", () => {
  ...
  it("verifies every deployed JSON body byte-for-byte against reviewed sources", async () => {
    ...
    assert.deepEqual(observation.counts, { total: 13, chain114: 12, simulation: 2 });
    ...
    assert.equal(evidence.corpus.entries.length, 13);
  });
